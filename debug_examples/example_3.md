...
```
Данный пример кода был скопирован у одного из студентов. 
Приводится тут только с целью проведения работы над ошибками в работе программы.
Он не является образцом для подражания - в нем хватает разного рода ошибок.
Статья не преследует своей целью отрефакторить данный код.
```
...
(Как можно зафиксировать значения переменных рядом с кодом - см [example_1](example_1.md)  Закрепление значения переменных в IDE)
