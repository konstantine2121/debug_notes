...
Являются магическими числами - ибо не ясно что эти значения обозначают, более того они используются в разных местах программы в разных обстоятельствах.
...
Также Следует установить галочку на "Продолжить выполнение кода", чтобы работа программы не прерывалась.
...
6 - выступает ограничителем по строке для первого множителя
2 - выступает начальным значением для первого множителя
...
10 - выступает ограничителем по строке для первого множителя
6 - выступает начальным значением для первого множителя
2 - начальное значение для второго множителя
33 - номер операции на которой произойдет перенос 2 ой группы 


Строка 90, multiplicationResult = 45

![](attachments/Pasted%20image%2020240319030157.png)

firstMultiplier = 6 - из-за того что значения переменных выводятся в лог уже после того как произошли изменения в блоке кода

![](attachments/Pasted%20image%2020240319030343.png)
...
Как мы помним меняется оно только в одном месте - и это связано с выводом на экран
...
Смотрим на строку инициализации
...
У настоящих программистов счет начинается с 0!
...
Смотрим еще раз на вывод программы

![](attachments/Pasted%20image%2020240319031947.png)

Если подсчитать для одной группы количество столбцов = 4
А также количество строк = 8
...
![](attachments/Pasted%20image%2020240319033612.png)

## Итог промежуточного рефакторинга без изменения логики
...
После того как осознали полностью логику работы программы можно попробовать ее переписать в более привычный вид
...
Вместо сложных условий которые опирались на переменные, которые изменялись где то еще... (и т.д.) мы пришли к использованию классических циклов счетчиков.

Вывод программы при этом не изменился.
